...
export default function ScanPage() {
  ...
  const [scanType, setScanType] = useState("image"); // image | url
  ...
  async function startScan() {
    if (scanType === "image" && !file) { setError("Please select an image first"); return; }
  ...
  return (
    <div className="w-full max-w-[900px] mx-auto space-y-6 relative">
      ...
      <div>
        ...
        <p className="text-[12px] text-[#64748B] font-medium">Upload a product image or paste a link for AI compliance analysis</p>
      </div>
      ...
      {(mode === "idle" || mode === "error") && (
        <>
          {/* Scan Type Selector */}
          <div className="bg-white rounded-2xl border border-[#D9E1EC] p-1.5 flex gap-1.5 shadow-sm">
            <button onClick={() => setScanType("image")}
              className={`flex-1 py-3 rounded-xl text-[13px] font-bold transition-all flex items-center justify-center gap-2 ${scanType === "image" ? "bg-[#1557C0] text-white shadow-md" : "text-[#64748B] hover:bg-[#F5F7FB]"}`}>
              <span className="text-[16px]">📷</span> Upload Image
            </button>
            <button onClick={() => setScanType("url")}
              className={`flex-1 py-3 rounded-xl text-[13px] font-bold transition-all flex items-center justify-center gap-2 ${scanType === "url" ? "bg-[#1557C0] text-white shadow-md" : "text-[#64748B] hover:bg-[#F5F7FB]"}`}>
              <span className="text-[16px]">🔗</span> Paste Link / URL
            </button>
          </div>
          ...
          {scanType === "image" ? (
            <div className="bg-white rounded-2xl border border-[#D9E1EC] p-6 shadow-sm">
              <h3 className="text-[13px] font-bold text-[#10264A] mb-1 flex items-center gap-2"><span>📷</span> Upload Package Image</h3>
              <p className="text-[11px] text-[#64748B] mb-4">Drag and drop or click to select a product package photo</p>
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all ${
                  dragOver ? "border-[#1557C0] bg-blue-50" : preview ? "border-[#16A34A] bg-green-50" : "border-[#D9E1EC] hover:border-[#1557C0] hover:bg-blue-50/30"
                }`}>
                <input ref={fileRef} type="file" accept="image/*" className="hidden"
                  onChange={e => handleFileSelect(e.target.files?.[0])} />
                {preview ? (
                  <div className="flex flex-col items-center gap-3">
                    <img src={preview} alt="Preview" className="max-h-56 rounded-xl shadow-md object-contain" />
                    <p className="text-[13px] text-[#16A34A] font-bold">✓ {file?.name}</p>
                    <p className="text-[10px] text-[#64748B]">Click to change image</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-3 text-[#64748B]">
                    <span className="text-6xl">📦</span>
                    <p className="text-[14px] font-bold text-[#10264A]">Drop package image here or click to browse</p>
            ...
            <div className="bg-white rounded-2xl border border-[#D9E1EC] p-6 shadow-sm">
              ...
              <p className="text-[11px] text-[#64748B] mb-4">Paste a product page link (e.g. Amazon, Flipkart, BigBasket, or any online store)</p>
              ...
              <p className="text-[10px] text-[#64748B] mt-3">The AI will scrape the product page, extract images and declarations, and check compliance automatically.</p>
            </div>
          )}
          ...
          <button onClick={startScan} disabled={scanType === "image" ? !file : !productUrl.trim()}
            ...
            <span className="text-[18px]">🔬</span> Start AI Compliance Scan
          </button>
        </>
      )}
      ...
      {mode === "processing" && (
        <div className="bg-white rounded-2xl border border-[#D9E1EC] p-8 shadow-sm">
          <div className="text-center mb-8">
            ...
            <h2 className="text-[18px] font-black text-[#10264A]">AI Analysis in Progress</h2>
            <p className="text-[12px] text-[#64748B] mt-1">Processing your {scanType === "url" ? "product URL" : "package image"}...</p>
          </div>
            ...
            {STEPS.map((step) => (
              <div key={step.id} className={`flex items-center gap-4 p-3 rounded-xl transition-all ${
                ...
                <div>
                  ...
                  {currentStep === step.id && <p className="text-[10px] text-[#1557C0] mt-0.5">Processing...</p>}
                  {currentStep > step.id && <p className="text-[10px] text-[#16A34A] mt-0.5">Completed</p>}
                </div>
              </div>
            ))}
          </div>
          <p className="text-center text-[10px] text-[#64748B] mt-6">First scan may take 30-60s while AI model initializes</p>
        </div>
      )}
        ...
        <>
          ...
          <div className={`bg-white rounded-2xl p-6 border-2 ${scoreBg} flex items-center gap-6 shadow-sm`}>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button onClick={() => { setMode("idle"); setResult(null); setFile(null); setPreview(null); setProductUrl(""); }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-[#D9E1EC] p-6 shadow-sm">
              <h3 className="text-[13px] font-bold text-[#10264A] mb-4">Scanned Image</h3>
              {preview && <img src={preview} alt="Scanned package" className="w-full max-h-56 object-contain rounded-xl" />}
