          ...
          {DEMO_PRODUCTS.map((p) => (
            <div key={p.id} className="card p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-lg">\u{1F4E6}</div>
                ...
                }`}>{p.status === "compliant" ? "\u2713 Compliant" : p.status === "flagged" ? "\u{1F6A9} Flagged" : "\u26A0 Review"}</span>
              </div>
